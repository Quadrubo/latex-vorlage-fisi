...
\begin{itemize}
	\item Beschreibung und Begründung der gewählten Anwendungsarchitektur (\zB \ac{MVC}).
...
\paragraph{Beispiel}
Anhand der Entscheidungsmatrix in Tabelle~\ref{tab:Entscheidungsmatrix} wurde für die Implementierung der Anwendung das \ac{PHP}-Framework Symfony\footnote{\Vgl ja}ausgewählt. 
...
\begin{itemize}
	\item Entwurf/Beschreibung der Datenstrukturen (\zB \acs{ERM} und/oder Tabellen\-modell, \acs{XML}-Sche\-mas) mit kurzer Beschreibung der wichtigsten (!) verwendeten Entitäten.
\end{itemize}
